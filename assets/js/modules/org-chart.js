...
MKNexus.OrgChartModule = (function () {
  ...
  function template() {
    return `
      <div class="orgchart-module" dir="rtl">
        <div class="orgchart-module__header">
          <span class="type-eyebrow orgchart-module__eyebrow">ORG CHART</span>
          <h1 class="orgchart-module__title">الهيكل التنظيمي</h1>
          <p class="orgchart-module__subtitle">قطاع ← إدارة ← منطقة — مين تحت مين، وتحديد اللي يشوفوا تقارير مين</p>
        </div>

        <div class="orgchart-columns">
          <div class="orgchart-column">
            <div class="orgchart-column__head">
              <span class="orgchart-column__title">القطاعات</span>
              <button class="orgchart-column__add" type="button" id="orgSectorsAddBtn" title="إضافة قطاع"><i class="fa-solid fa-plus"></i></button>
            </div>
            <div class="orgchart-column__list" id="orgSectorsList"></div>
          </div>
          <div class="orgchart-column">
            <div class="orgchart-column__head">
              <span class="orgchart-column__title">الإدارات</span>
              <button class="orgchart-column__add" type="button" id="orgAdminsAddBtn" title="إضافة إدارة"><i class="fa-solid fa-plus"></i></button>
            </div>
            <div class="orgchart-column__list" id="orgAdminsList"></div>
          </div>
          <div class="orgchart-column">
            <div class="orgchart-column__head">
              <span class="orgchart-column__title">المناطق</span>
              <button class="orgchart-column__add" type="button" id="orgRegionsAddBtn" title="إضافة منطقة"><i class="fa-solid fa-plus"></i></button>
            </div>
        ...
        <div class="orgchart-members">
          <div class="orgchart-members__head">
            <div>
              <div class="orgchart-members__title" id="orgMembersTitle">اختر قطاع، إدارة، أو منطقة</div>
            ...
            <div class="orgchart-members__assign" id="orgAssignRow" hidden>
              ...
              <button class="btn btn--primary" type="button" id="orgAssignBtn"><i class="fa-solid fa-user-plus"></i><span>إضافة</span></button>
            </div>
          </div>
        ...
        <div class="orgchart-modal" id="orgNameModal">
          <div class="orgchart-modal__panel">
            <div class="orgchart-modal__title" id="orgNameModalTitle">إضافة</div>
            ...
            <div class="orgchart-modal__actions">
              <button class="btn btn--ghost" type="button" id="orgNameModalCancelBtn">إلغاء</button>
              <button class="btn btn--primary" type="button" id="orgNameModalSaveBtn">حفظ</button>
            </div>
          </div>
        </div>
        ...
        <div class="orgchart-loader" id="orgLoader">
          ...
          <div class="orgchart-loader-text" id="orgLoaderText">جاري التنفيذ...</div>
        </div>
      </div>`;
  }
  ...
  function loadAll() {
    showLoader('جاري تحميل الهيكل التنظيمي...');
    return Promise.all([MKNexus.ApiClient.getOrgStructure(), MKNexus.ApiClient.getUsers()])
      ...
      .catch((error) => {
        hideLoader();
        MKNexus.Toast.error(error?.message || 'تعذر تحميل الهيكل التنظيمي');
      });
  }
    ...
    return `
      <div class="orgchart-node ${isSelected ? 'is-selected' : ''}" data-id="${escapeHtml(id)}">
        ...
        <div class="orgchart-node__actions">
          <button class="orgchart-icon-btn" type="button" data-action="rename" title="إعادة تسمية"><i class="fa-solid fa-pen"></i></button>
          <button class="orgchart-icon-btn orgchart-icon-btn--danger" type="button" data-action="delete" title="حذف"><i class="fa-solid fa-trash"></i></button>
        </div>
      </div>`;
  }
  ...
  function renderSectors() {
    if (!sectors.length) {
      sectorsListEl.innerHTML = '<div class="orgchart-column__empty">لا يوجد قطاعات — ابدأ بإضافة واحد</div>';
      return;
    }
  ...
  function renderAdministrations() {
    ...
    if (!selectedSectorId) {
      adminsListEl.innerHTML = '<div class="orgchart-column__empty">اختر قطاع الأول</div>';
      return;
    }
    const rows = administrations.filter((a) => String(a.SectorID) === String(selectedSectorId));
    if (!rows.length) {
      adminsListEl.innerHTML = '<div class="orgchart-column__empty">لا يوجد إدارات تحت هذا القطاع</div>';
      return;
    }
  ...
  function renderRegions() {
    ...
    if (!selectedAdministrationId) {
      regionsListEl.innerHTML = '<div class="orgchart-column__empty">اختر إدارة الأول</div>';
      return;
    }
    const rows = regions.filter((r) => String(r.AdministrationID) === String(selectedAdministrationId));
    if (!rows.length) {
      regionsListEl.innerHTML = '<div class="orgchart-column__empty">لا يوجد مناطق تحت هذه الإدارة</div>';
      return;
    }
  ...
  function memberRowHtml(u, onRemove) {
    return `
      <div class="orgchart-member-row" data-user-id="${escapeHtml(u.ID || '')}">
        ...
        <button class="orgchart-icon-btn orgchart-icon-btn--danger" type="button" data-action="${onRemove}" title="إزالة"><i class="fa-solid fa-user-minus"></i></button>
      </div>`;
  }
  ...
  function renderMembers() {
    ...
    if (!level) {
      membersHeadEl.textContent = 'اختر قطاع، إدارة، أو منطقة';
    ...
    if (level === 'sector') {
      ...
      title = `مسؤول القطاع: ${sector?.Name || ''}`;
      ...
      title = `مديرو الإدارة: ${admin?.Name || ''}`;
      ...
      const sector = sectors.find((s) => String(s.ID) === String(admin?.SectorID));
      title = `أعضاء المنطقة: ${region?.Name || ''}`;
    ...
    membersListEl.innerHTML = members.length
      ...
      : '<div class="orgchart-column__empty">لا يوجد أعضاء بعد</div>';
    ...
    assignSelectEl.innerHTML = candidates.length
      ...
      : '<option value="">لا يوجد مرشحين متاحين</option>';
  ...
  function assignMember() {
    ...
    MKNexus.ApiClient.updateUser({ id: userId, ...updates })
      .then(() => { hideLoader(); MKNexus.Toast.success('تمت الإضافة'); return loadAll(); })
      .catch((error) => { hideLoader(); MKNexus.Toast.error(error?.message || 'تعذرت الإضافة'); });
  }
  ...
  function removeMember(userId) {
    ...
    showLoader('جاري الإزالة...');
    MKNexus.ApiClient.updateUser({ id: userId, ...updates })
      .then(() => { hideLoader(); MKNexus.Toast.success('تمت الإزالة'); return loadAll(); })
      .catch((error) => { hideLoader(); MKNexus.Toast.error(error?.message || 'تعذرت الإزالة'); });
  }
  ...
  const LEVEL_META = {
    sector: {
      ...
      createTitle: 'إضافة قطاع', renameTitle: 'إعادة تسمية القطاع',
      confirmDelete: (name) => `تأكيد حذف القطاع "${name}"؟`,
    },
    administration: {
      ...
      createTitle: 'إضافة إدارة', renameTitle: 'إعادة تسمية الإدارة',
      confirmDelete: (name) => `تأكيد حذف الإدارة "${name}"؟`,
    },
    region: {
      ...
      createTitle: 'إضافة منطقة', renameTitle: 'إعادة تسمية المنطقة',
      confirmDelete: (name) => `تأكيد حذف المنطقة "${name}"؟`,
    },
  };
  ...
  function saveNameModal() {
    ...
    if (!name) { MKNexus.Toast.warning('من فضلك اكتب اسم'); return; }
    ...
    request
      .then(() => { hideLoader(); MKNexus.Toast.success('تم الحفظ'); closeNameModal(); return loadAll(); })
      .catch((error) => { hideLoader(); MKNexus.Toast.error(error?.message || 'حدث خطأ أثناء الحفظ'); });
  }
  ...
  function deleteNode(level, id, name) {
    ...
    meta.remove(id)
      .then(() => {
        hideLoader();
        MKNexus.Toast.success('تم الحذف');
      ...
      .catch((error) => {
        hideLoader();
        // HAS_DEPENDENTS from org-structure.gs's delete guards — surfaced
        // as-is, it's already a clear Arabic-agnostic instruction.
        MKNexus.Toast.error(error?.message || 'تعذر الحذف');
      });
  }
